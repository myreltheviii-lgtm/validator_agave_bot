...
pub fn execute(
    ...
    //
    // These values flow from the CLI flags defined in commands/run/args.rs into
    // the ValidatorConfig struct fields defined in solana-core/src/validator.rs.
    // From there, Validator::new reads them during startup, constructs a
    // MevStartupConfig, and passes it to initialize_mev_components, which
    // performs the wallet load, RPC client construction, LUT fetch, and pool
    // scan before spawning the "solMevEngine" OS thread.
    //
    // When mev_enabled is false (--mev-enabled absent), none of the other fields
    // are consulted — the MevEngine block inside Validator::new is skipped
    // entirely, so it is safe to leave them as empty/default values.
    // -------------------------------------------------------------------------
    let mev_enabled = matches.is_present("mev_enabled");

    // PathBuf::from("") is a valid but unreachable value when mev_enabled is
    // false. initialize_mev_components will return an error before using it if
    // the path is empty and the engine is somehow started, so no silent failures
    // can occur even if this default is used unexpectedly.
    let mev_wallet_path = matches
        .value_of("mev_wallet_path")
        .map(PathBuf::from)
        .unwrap_or_default();

    let mev_rpc_url = matches
        .value_of("mev_rpc_url")
        .unwrap_or_default()
        .to_string();

    // A u64 priority fee with a zero default is safe: with mev_enabled=false the
    // engine never runs, and a zero fee is caught by the profitability check
    // inside ArbitrageExecutor before any transaction is submitted.
    let mev_base_priority_fee = value_t!(matches, "mev_base_priority_fee", u64).unwrap_or(0);

    // When --mev-validation-mode is present the engine simulates and logs
    // opportunities but does not send transactions to the cluster. Absent means
    // live trading mode.
    ...
    // are provided, which is a valid configuration (engine runs without LUT
    // compression; only single-leg paths within the 35-account limit are viable).
    let mev_lut_addresses: Vec<Pubkey> = matches
        .values_of("mev_lut_address")
        .into_iter()
        .flatten()
        .map(|s| s.parse::<Pubkey>().expect("mev_lut_address already validated by clap"))
        .collect();

    // The minimum profit floor, in lamports, that the on-chain SMB executor enforces
    // before permitting a swap to succeed. The engine encodes this u64 into instruction
    // data bytes [1..9] (little-endian). If the realised swap output falls below this
    // value the program reverts the entire transaction — no net loss is taken even if
    // pool prices moved between simulation and landing. The simulation pass always
    // passes zero so the SVM executes the full path regardless of current prices;
    // only the final submission transaction carries this non-zero threshold.
    ...
    // as the leader produces them — before the canonical replay pipeline has
    // processed or even received them. An empty string is safe when mev_enabled is
    // false because the bridge task is never spawned unless the engine thread starts.
    let mev_shredstream_url = matches
        .value_of("mev_shredstream_url")
        .unwrap_or_default()
        .to_string();
