...
pub fn add_args<'a>(app: App<'a, 'a>, default_args: &'a DefaultArgs) -> App<'a, 'a> {
    ...
    // -------------------------------------------------------------------------
    // MEV arbitrage engine configuration
    //
    // These flags activate and parameterise the speculative arbitrage subsystem
    // that runs inside the validator process on the "solMevEngine" OS thread.
    // The engine is fully isolated from consensus: it uses its own Tokio runtime
    // ("solMevArb00-03"), its own wallet keypair, and its own RPC connection.
    // It does not affect block production, replay, or vote submission in any way.
    //
    // None of these flags have defaults — omitting --mev-enabled is the safe
    // default (engine disabled). Omitting the other flags while --mev-enabled is
    // set causes `initialize_mev_components` to fail at startup with a clear
    // error message identifying the missing argument.
    // -------------------------------------------------------------------------
    .arg(
        Arg::with_name("mev_enabled")
            .long("mev-enabled")
            .takes_value(false)
            .help(
                "Enable the integrated MEV arbitrage engine. When this flag is absent \
                 the engine is not started — no wallet is loaded, no pool scan runs, \
                 and the solMevEngine thread is never spawned.",
            ),
    )
    .arg(
        Arg::with_name("mev_wallet_path")
            .long("mev-wallet-path")
            .value_name("PATH")
            .takes_value(true)
            ...
            .validator(|s| {
                if std::path::Path::new(&s).exists() {
                    ...
                    Err(format!("mev-wallet-path '{}' does not exist", s))
                }
            })
            .help(
                "Path to the JSON keypair file used to sign and fund arbitrage transactions. \
                 The account must hold enough SOL to cover transaction fees and the wSOL \
                 buffer required for pool swaps. Required when --mev-enabled is set.",
            ),
    )
    .arg(
        Arg::with_name("mev_rpc_url")
            .long("mev-rpc-url")
            .value_name("URL")
            .takes_value(true)
            .requires("mev_enabled")
            .help(
                "JSON-RPC endpoint used by the MEV engine for transaction submission and \
                 for fetching address lookup table accounts during startup. Pointing this \
                 at the local validator RPC port (e.g. http://127.0.0.1:8899) avoids \
                 external network hops on the critical submission path. Required when \
                 --mev-enabled is set.",
            ),
    )
    .arg(
        Arg::with_name("mev_base_priority_fee")
            .long("mev-base-priority-fee")
            .value_name("MICROLAMPORTS_PER_CU")
            .takes_value(true)
            .requires("mev_enabled")
            .validator(|s| is_parsable::<u64>(s))
            .help(
                "Minimum compute-unit priority fee, in microlamports, attached to every \
                 arbitrage transaction. Acts as a floor: the engine will not submit a trade \
                 whose expected profit does not cover at least this fee times the estimated \
                 compute units. Higher values reduce submission noise on a busy network \
                 but also reduce the set of profitable opportunities. Required when \
                 --mev-enabled is set.",
            ),
    )
    .arg(
        Arg::with_name("mev_validation_mode")
            .long("mev-validation-mode")
            .takes_value(false)
            .requires("mev_enabled")
            .help(
                "Run the MEV engine in validation mode: simulations execute and profit \
                 calculations run, but no transactions are submitted to the network. \
                 Useful for confirming that the engine finds opportunities and computes \
                 fees correctly before enabling live trading.",
            ),
    )
    .arg(
    ...
    .arg(
        Arg::with_name("mev_lut_address")
            ...
            .help(
                "On-chain address of an address lookup table to load at startup. Each LUT \
                 encodes pool vault, tick-array, and oracle pubkeys so they can be \
                 referenced as 1-byte indices in versioned (v0) transactions rather than \
                 32-byte pubkeys, freeing account-list space for additional legs. Pass \
                 this flag up to 10 times. Without LUTs, complex multi-pool paths may \
                 exceed the 35-account static limit and be dropped silently.",
            ),
    )
    .arg(
        Arg::with_name("mev_min_profit_lamports")
            .long("mev-min-profit-lamports")
            .value_name("LAMPORTS")
            .takes_value(true)
            .requires("mev_enabled")
            .validator(|s| is_parsable::<u64>(s))
            .help(
                "Minimum profit in lamports that a simulated two-hop arbitrage must yield \
                 before the engine submits the transaction on-chain. This value is encoded \
                 into the SMB instruction at bytes [1..9] (little-endian u64) and is read \
                 by the on-chain executor after it computes the swap output — the program \
                 reverts atomically if the output falls below this threshold, so unprofitable \
                 trades never land as net losses. The simulation pass always uses zero so the \
                 SVM runs the full execution path regardless of price; only the final \
                 submission transaction carries this floor. Calibrate this value against \
                 current priority fees and the cost of a failed transaction landing. \
                 Required when --mev-enabled is set.",
            ),
    )
    .arg(
    ...
    .arg(
        Arg::with_name("mev_shredstream_url")
            ...
            .requires("mev_enabled")
            .help(
                "gRPC endpoint of the Jito ShredStream proxy that delivers individual \
                 shreds to this validator as the slot leader broadcasts them. The MEV \
                 engine subscribes to this stream and runs speculative SVM execution on \
                 each arriving entry batch before the canonical replay pipeline has \
                 processed it, giving the engine a latency advantage of 200-400ms per \
                 slot over bots that can only observe confirmed account state. Example: \
