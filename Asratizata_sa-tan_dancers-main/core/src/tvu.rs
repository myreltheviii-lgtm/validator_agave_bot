...
impl Tvu {
    ...
    pub fn new(
        ...
        // Delivers an already-frozen Arc<Bank> to the MEV engine the instant
        // bank.freeze() completes inside process_replay_results.  Every frozen
        // bank travels this channel before being handed to any other downstream
        // subscriber (bank_notification_sender, cost_update_sender, etc.) so
        // the engine can update its canonical fallback bank and rebase any
        // speculative predictions with the minimum possible latency.  The Tvu
        // layer simply threads this sender through to ReplaySenders; it does
        // not own or consume the channel.
        mev_frozen_bank_sender: crossbeam_channel::Sender<Arc<solana_runtime::bank::Bank>>,
        // Delivers the slot number of every slot that mark_dead_slot rejects to
        // the MEV engine.  A slot is dead when canonical replay fails due to an
        // invalid PoH hash chain, failed Ed25519 batch verification, SVM
        // execution error, or chained block-ID mismatch.  On receipt the engine
        // evicts the speculative bank for that slot and every speculative
        ...
        let replay_senders = ReplaySenders {
            ...
            lockouts_sender,
            // Thread the MEV channels from the parameter list straight into
            // ReplaySenders.  ReplayStage is the sole consumer of both channels
