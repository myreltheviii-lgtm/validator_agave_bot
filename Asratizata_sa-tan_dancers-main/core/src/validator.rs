...
use {
    crate::{
        ...
        mev::engine::MevEngine,
        mev::{initialize_mev_components, MevStartupConfig},
    },
    agave_snapshots::{
...
pub struct ValidatorConfig {
    ...
    //
    // These fields are read by `initialize_mev_components` during `Validator::new`
    // and forwarded to `MevEngine::new`. They are intentionally separate from the
    // Jito relayer/block-engine fields above because the MEV engine is an independent
    // arbitrage subsystem with its own wallet, RPC endpoint, and operational mode.
    /// When false, the MEV engine is not started at all — no wallet file is read,
    /// no pool scan runs, and the `"solMevEngine"` thread is never spawned.
    pub mev_enabled: bool,
    /// Path to the MEV wallet keypair JSON file used to sign and fund arbitrage transactions.
    pub mev_wallet_path: PathBuf,
    /// JSON-RPC endpoint used by the MEV engine for pool discovery and transaction submission.
    pub mev_rpc_url: String,
    /// Minimum per-CU priority fee (in microlamports) attached to every MEV transaction.
    /// Acts as a floor — arbitrage transactions below this fee are not submitted.
    pub mev_base_priority_fee: u64,
    /// When true, the MEV engine validates that simulated arbitrage profits exceed
    /// expected fees before submitting. Set to false only during integration testing.
    pub mev_validation_mode: bool,
    /// When true, `MevEngine` compares post-rebase speculative account values against
    /// the corresponding frozen canonical bank values after every `confirm_slot` call.
    /// Mismatches are logged as warnings.  This is a diagnostic flag for validating
    /// the correctness of speculative execution against canonical replay.  It adds
    /// O(watched_accounts) bank reads per slot freeze (~2.5/s) and should be disabled
    /// in production unless actively investigating accuracy issues.
    pub mev_speculative_accuracy_check: bool,
    /// Minimum profit in lamports that a simulated arbitrage must yield before
    /// the MEV engine submits the transaction on-chain.  Arbitrage paths that
    /// do not clear this threshold after subtracting estimated fee cost are
    /// discarded.  Setting this too low wastes transaction fees on unprofitable
    /// trades; setting it too high leaves money on the table.  The value should
    ...
    /// they are assembled into full entries and replayed by the canonical
    /// pipeline.  This lets the engine run speculative transaction execution
    /// on partial entry batches and detect arbitrage opportunities in the same
    /// slot they are created, rather than one slot later.
    pub mev_shredstream_url: String,
    /// Address lookup tables loaded at startup and held for the engine's lifetime.
    ///
    /// Solana's base transaction format can reference at most 35 static accounts.
    /// A two-leg arbitrage across two CLMM or Whirlpool pools can easily require
    /// 28–32 accounts (pool state, vaults A/B, tick arrays, oracles, token program,
    /// associated token program, wallet, two ATAs, etc.). Without LUT compression
    /// that leaves almost no headroom, and a three-leg path is impossible.
    ///
    /// A versioned (v0) transaction replaces each account that appears in a LUT with
    /// a 1-byte index instead of a 32-byte pubkey, recovering roughly 31 bytes per
    /// account and making complex paths fit within the 1232-byte MTU. Up to four
    /// LUTs may be attached per transaction; `LutManager` selects the subset that
    /// covers the most accounts for each individual transaction.
    ///
    /// Each entry in this vector is the on-chain address of a pre-deployed lookup
    /// table. The accounts inside each table are fetched from the cluster during
    /// `initialize_mev_components` and cached for the process lifetime. LUT state
...
impl ValidatorConfig {
    pub fn default_for_test() -> Self {
        Self {
            ...
            // MEV defaults — callers that use `default_for_test()` will not exercise
            // the MEV engine; these values prevent compilation errors only.
            mev_enabled: false,  // never start MEV in tests
            mev_wallet_path: PathBuf::from("wallet.json"),
            mev_rpc_url: "http://127.0.0.1:8899".to_string(),
            mev_base_priority_fee: 1_000,
            mev_validation_mode: true,
            // Accuracy checking is off by default — it adds bank reads per freeze slot.
            // Enable only when diagnosing speculative-vs-canonical divergence.
            mev_speculative_accuracy_check: false,
            // A floor of 5_000 lamports covers a typical 5000-CU arbitrage
            ...
            // No LUTs in test contexts — the MEV engine is disabled anyway, and
...
pub struct Validator {
    ...
    /// other post-freeze subscriber.  Stored here so the channel remains live for
    /// the entire validator lifetime; dropping it would disconnect the engine from
    /// canonical state updates.  None when MEV is disabled.
    mev_frozen_bank_sender: Option<crossbeam_channel::Sender<Arc<Bank>>>,
    /// Sender half of the dead-slot channel wired from ReplayStage into the MEV
    /// engine.  ReplayStage calls send() from mark_dead_slot whenever canonical
    /// replay permanently rejects a slot.  Stored here so the channel remains live
    /// for the entire validator lifetime.  None when MEV is disabled.
    mev_dead_slot_sender: Option<crossbeam_channel::Sender<Slot>>,
}
...
impl Validator {
    ...
    pub fn new_with_exit(
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::CleaningAccounts;
        ...
        // ─────────────────────────────────────────────────────────────────────
        // MEV ENGINE INITIALISATION
        //
        // The MEV engine is an entirely independent subsystem — it runs on its
        // own OS thread ("solMevEngine") with its own 4-thread Tokio runtime
        ...
        // ReplayStage, which is the only component that writes to them.
        //
        // If `config.mev_enabled` is false the MEV engine is never started:
        ...
        // at most once per slot (~400 ms) and the consumer processes each bank
        // in microseconds.  Back-pressure is not a concern at this frequency.
        let (mev_frozen_bank_tx, mev_frozen_bank_rx) =
            crossbeam_channel::unbounded::<Arc<Bank>>();

        // Dead slot channel: ReplayStage → MevEngine.
        // Dead slots are rare (only occur on invalid PoH, bad Ed25519 batch,
        // SVM execution error, or chained block-ID mismatch) so an unbounded
        ...
        let mev_engine_thread = if config.mev_enabled {
            let mev_startup_config = MevStartupConfig {
                wallet_path: config.mev_wallet_path.clone(),
                rpc_url: config.mev_rpc_url.clone(),
                lut_addresses: config.mev_lut_addresses.clone(),
                // initialize_mev_components logs and validates the shredstream
                // endpoint during the one-time startup sequence so any
                // misconfigured URL is caught before the engine thread starts.
                // The same URL is also forwarded separately to MevEngine::new()
                // below because MevStartupResult carries only constructed
                // resources (wallet, RPC client, LUT manager, pool data) and
                // not configuration scalars — the bridge task that actually
                // opens the gRPC stream is spawned later inside run_async().
                shredstream_url: config.mev_shredstream_url.clone(),
            };
            let mev_startup_result = initialize_mev_components(&bank_forks, &mev_startup_config)
            ...
            Some(
                std::thread::Builder::new()
                    ...
                    .expect("failed to spawn solMevEngine thread"),
            )
        } else {
        ...
        *admin_rpc_service_post_init.write().unwrap() = Some(AdminRpcRequestMetadataPostInit {
        ...
        Ok(Self {
            ...
            // dropping the last sender causes the engine's recv() to return
            // Disconnected.  Both senders are intentionally dropped in join()
            // before tpu.join() so the engine exits cleanly before the Tpu
            // and its shredstream sender are torn down.
            mev_frozen_bank_sender: Some(mev_frozen_bank_tx),
            mev_dead_slot_sender: Some(mev_dead_slot_tx),
        })
    }
    ...
    pub fn join(self) {
        ...
        // clones are gone.  Dropping our copies here first guarantees that once
        // the engine sees Disconnected on either channel, no further sends are
        // possible from any thread — the engine can exit its select! loop cleanly
        // without racing against a late send from a partially-torn-down ReplayStage.
        drop(self.mev_frozen_bank_sender);
        drop(self.mev_dead_slot_sender);
        ...
        // Join the MEV engine thread after Tpu has fully shut down (above).
        //
        // At this point: our sender was dropped before tpu.join, Tpu.join()
        // completed (Tpu dropped its sender clone), so MevEngine has seen
        // Disconnected and exited run(). The join here is therefore non-blocking
        // in practice — it merely waits for the OS thread to be recycled.
        if let Some(thread) = self.mev_engine_thread {
            thread.join().expect("mev_engine_thread");
        }
    }
}
