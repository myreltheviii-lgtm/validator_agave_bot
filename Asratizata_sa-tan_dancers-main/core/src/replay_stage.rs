...
pub struct ReplaySenders {
    ...
    /// Carries a fully-frozen `Arc<Bank>` to the MEV engine the instant
    /// `bank.freeze()` completes inside `process_replay_results`.  The engine
    /// uses this to update its `canonical_bank` fallback and to rebase any
    /// speculative child banks that were built on top of this slot's predicted
    /// account state — producing correction deltas that are immediately fed
    /// back into the speculative update handler.  Because this send happens
    /// before any other post-freeze work (bank_notification_sender,
    /// cost_update_sender, etc.) the engine receives the bank at the earliest
    /// possible moment with zero coupling to RPC or transaction-status services.
    pub mev_frozen_bank_sender: crossbeam_channel::Sender<Arc<Bank>>,
    /// Carries the slot number of every slot that `mark_dead_slot` rejects so
    /// that the MEV engine can evict the corresponding speculative bank and all
    /// of its descendants.  Without this signal the engine would continue
    /// simulating arbitrage against account state that the canonical replay
...
impl ReplayStage {
    ...
    fn mark_dead_slot(
        ...
        replay_vote_sender: &ReplayVoteSender,
        // When Some, the dead slot number is forwarded to the MEV engine so
        // it can evict the corresponding speculative bank and all descendants
        // before they can be used for arbitrage simulation.  None is passed
        // only by unit tests that call this function directly without a live
        // MEV channel.
        ...
        // Notify the MEV engine that this slot is permanently invalid.  A slot
        // is dead when replay fails due to an invalid PoH chain, failed Ed25519
        // batch verification, SVM execution error, or chained block-ID
    ...
    fn process_replay_results(
        ...
        // Carries the fully-frozen Arc<Bank> to the MEV engine the moment
        // bank.freeze() completes — before bank_notification_sender and all
        // other post-freeze work — so the engine can update its canonical
        // fallback bank and rebase speculative predictions with zero coupling
        // to RPC or transaction-status latency.
        mev_frozen_bank_sender: &crossbeam_channel::Sender<Arc<Bank>>,
        // Carries dead slot numbers to the MEV engine so it can discard
        // speculative banks built on rejected canonical entries.
        ...
        for replay_result in replay_result_vec {
            ...
            if bank.is_complete() {
                ...
                // Push the now-immutable bank directly to the MEV engine.
                // This send happens before every other post-freeze subscriber
                // (bank_notification_sender, cost_update_sender, votor, etc.)
                // so the engine gets the bank at the earliest possible moment.
                // The engine uses it to update canonical_bank — the fallback
                // used by ArbitrageExecutor when no speculative bank covers a
                // slot — and to call confirm_slot, which rebases speculative
    ...
    fn replay_active_banks(
        ...
        migration_status: &MigrationStatus,
        votor_event_sender: &VotorEventSender,
        // Forwarded directly into process_replay_results where the frozen bank
        // send occurs immediately after bank.freeze().
        mev_frozen_bank_sender: &crossbeam_channel::Sender<Arc<Bank>>,
        ...
        let res = {
            ...
            if let Err(err) = &res {
                ReplayStage::mark_dead_slot(
                    ...
                    &replay_vote_sender,
                    // No MEV engine in tests — passing None skips the dead slot
                    // channel send without requiring a live receiver.
                    None,
                );
            }
            assert_eq!(
