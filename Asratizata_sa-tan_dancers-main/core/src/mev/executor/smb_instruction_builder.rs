...
impl SmbInstructionBuilder {
    ...
    fn add_pump_accounts(
        ...
        // The accumulator's wSOL ATA receives the rebate, so both it and the accumulator
        // itself must appear as writable accounts. These two accounts are appended only for
        // cashback-enabled tokens; standard Pump tokens omit them entirely.
