...
/// Timeout applied to each individual heartbeat gRPC call.
/// If the block engine is unresponsive, without a timeout the heartbeat thread blocks
/// inside runtime.block_on() and cannot process the shutdown_receiver arm.
                ...
                crossbeam_channel::select! {
                    ...
                    recv(heartbeat_tick) -> _ => {
                        ...
                        match runtime.block_on(async {
                            ...
                            Err(_elapsed) => {
                                warn!(
                                    "Heartbeat timed out after {HEARTBEAT_SEND_TIMEOUT:?}. \
                                     Block engine may be unresponsive."
                                );
                                datapoint_warn!(
                                    "shredstream_proxy-heartbeat_send_error",
                                    "block_engine_url" => block_engine_url,
                                    ("errors", 1, i64),
                                    ("error_str", "timeout".to_string(), String),
                                );
                                failed_heartbeat_count += 1;
                            }
                            // The gRPC call completed but returned an error status.
                            Ok(Err(err)) => {
