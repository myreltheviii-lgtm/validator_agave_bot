        ...
        tokio::spawn(async move {
            ...
            while !exit.load(Ordering::Relaxed) {
                ...
                if refresh_token_ttl < Duration::from_secs(5 * 60) {
                    ...
                    let is_error = match Self::auth(&mut auth_service_client, &keypair, role).await
                    {
                        Ok((new_access_token, new_refresh_token)) => {
                            // The server is required to provide expiry timestamps. If it does not,
                            // we treat this as an error and retain the current (still valid) tokens
                            // rather than panicking and terminating the refresh task. A panic here
                            // would silently stop all future token refreshes, eventually causing auth
                            // failures ~1 hour after the current access token expires.
                            match new_access_token.expires_at_utc {
                                Some(expiry) => {
                                    // Move the token value (no clone needed) into the new Arc.
                                ...
                                None => {
                                    log::warn!(
                                        "Full re-auth succeeded but access token has no expiry; \
                                         retaining current tokens."
                                    );
                                    true
                                }
                            }
                        ...
                        Err(e) => {
                            log::warn!("Full re-auth failed: {e}");
                            true
                        }
                    };
                    datapoint_info!(
                ...
                if access_token_ttl < Duration::from_secs(5 * 60) {
                    let start = Instant::now();
                    let is_error = match auth_service_client
                    ...
                    {
                        Ok(refresh_resp) => {
                            let access_token = refresh_resp.into_inner().access_token;
                            // Guard against a server that returns a token without an expiry.
                            // Panicking here would terminate the token refresh task silently.
                            match access_token.and_then(|t| t.expires_at_utc.map(|e| (t.value, e))) {
                                Some((token_value, expiry)) => {
                                    // Move the token value — avoids cloning a potentially large String.
                                    bearer_token.store(Arc::new(token_value));
                                    access_token_expiration = expiry;
                                    false
                                }
                                None => {
                                    log::warn!(
                                        "Access token refresh succeeded but token or expiry is missing; \
                                         retaining current token."
                                    );
                                    true
                                }
                            }
                        ...
                        Err(e) => {
                            log::warn!("Access token refresh failed: {e}");
                            true
                        }
                    };
